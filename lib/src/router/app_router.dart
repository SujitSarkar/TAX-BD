class AppRouter {
  static const String initializer = '/';
  static const String home = '/home';

  ///Auth Screen
  static const String signIn = '/signIn';
  static const String otpScreen = '/otp-screen';

  ///Home Screen
  static const String rebateCalculationScreen = '/rebate-calculation-screen';
  static const String taxCalculationScreen = '/tax-calculation-screen';
  static const String expenseInformationScreen = '/expense-information-screen';
  static const String personalInfoScreen = '/personal-info-screen';
  static const String assetInfoScreen = '/asset-info-screen';

  ///Income Screens
  static const String incomeInfoScreen = '/income-info-screen';
  static const String govtSalaryIncomeScreen = '/govt-salary-income-screen';
  static const String privateSalaryIncomeScreen =
      '/private-salary-income-screen';
  static const String agricultureIncomeScreen = '/agriculture-income-screen';
  static const String businessIncomeScreen = '/business-income-screen';
  static const String financialAssetIncomeScreen = '/financial-asset-income-screen';
  static const String rentalIncomeScreen = '/rental-income-screen';
  static const String othersIncomeScreen = '/others-income-screen';
  static const String spouseChildrenIncomeScreen = '/spouse-children-income-screen';
  static const String foreignIncomeScreen = '/foreign-income-screen';
  static const String partnershipBusinessIncomeScreen = '/partnership-business-income-screen';
  static const String capitalGainIncomeScreen = '/capital-gain-income-screen';


}
